  ...
  if(isset($_POST['upate_product']))
    {
            ...
            {
                $ret="SELECT * FROM product where s_id='".$s_id."' and id='".$id."'"; //sql code to get to ten trains randomly
                if(isset($stmt)){
                $stmt->close();}
            ...
            $query="update product set pname=?, category=?, price=?, address=?, p_owner=?, p_status=?,description=?,p_quantity=?, p_img_loc=?,phone=? where id = ? and s_id=?";
            $stmt = $mysqli->prepare($query);
            $rc=$stmt->bind_param('ssssssssssii', $p_name, $p_category,$p_price,$address,$p_owner,$p_status,$p_des,$p_quantity,$p_pic_loc,$phone,$id,$s_id);
            $stmt->execute();
                if($stmt)
                {
                    $succ = "Product Update";
          ...
          <form method ="POST" enctype="multipart/form-data"> 
            ...
            <div class="form-group">
              ...
              <select class="form-control" name="p_owner" id="exampleFormControlSelect1">
                ...
                $ret="SELECT * FROM user where u_id='".$s_id."'"; //sql code to get to ten trains randomly
                if(isset($stmt)){
                $stmt->close();}
                $stmt= $mysqli->prepare($ret) ;
                $stmt->execute() ;//ok
